...
//Das hier kommt aufs erste raus
/*
...
*/
//hier der ram vom gatemate (Xaver)
module gatemate_ram #(
  ...
  always @(posedge clka) begin
    if (req_in_a) begin
      if (wea) begin
        memory[addr_in_a] <= data_in_a;
      end
      else begin
          data_out_a <= memory[addr_in_a];
      end
    end
  always @(posedge clkb) begin
    if (req_in_b) begin
      if (web) begin
        memory[addr_in_b] <= data_in_b;
      end
      else begin
          data_out_b <= memory[addr_in_b];
      end
    end
endmodule